...
1. **확률 해석 제한**: 별도의 확률 보정을 하지 않았으므로 위험도 점수를 실제 이탈 확률로 단정하지 않습니다.
2. **인과 해석 제한**: 중요 피처는 모델의 판단 신호이며 직접적인 이탈 원인을 의미하지 않습니다.
3. **날짜 피처 검증**: 예측 기준일에 계약 날짜 정보가 실제 원천 시스템에서 사용 가능한지 확인해야 합니다.
4. **캠페인 효과 미검증**: 위험 고객 포착과 실제 이탈 방어는 다르므로 A/B Test 또는 Uplift Modeling이 필요합니다.
5. **분포 변화 대응**: 운영 데이터의 고객 구성과 이탈률을 모니터링하고 재학습 기준을 수립해야 합니다.
