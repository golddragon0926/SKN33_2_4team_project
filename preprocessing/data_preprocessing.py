...
def find_project_root(start: str | Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        "프로젝트 루트를 찾을 수 없습니다. "
        "프로젝트 안에 data/raw 폴더를 만들고 "
        "client_data.csv와 price_data.csv를 넣어주세요."
    )
...
def load_raw_data(
    ...
    if not client_path.exists() or not price_path.exists():
        raise FileNotFoundError(
            "data/raw/client_data.csv 또는 "
            "data/raw/price_data.csv가 없습니다."
        )
    ...
    missing_client_cols = sorted(
        required_client_cols - set(client.columns)
    )
    missing_price_cols = sorted(
        required_price_cols - set(price.columns)
    )
    ...
    for col in CLIENT_DATE_COLS:
        client[col] = pd.to_datetime(
            client[col],
            errors="coerce",
        )

    price["price_date"] = pd.to_datetime(
        price["price_date"],
        errors="coerce",
    )
...
def validate_keys_and_population(
    client: pd.DataFrame,
    price: pd.DataFrame,
) -> dict[str, Any]:
    ...
    if set(price_model[ID_COL]) != client_ids:
        missing_price_customers = len(
            client_ids - set(price_model[ID_COL])
        )
        raise ValueError(
            "가격 이력이 없는 client_data 고객이 "
            f"{missing_price_customers}명 있습니다."
        )
...
def split_customer_data(
    ...
    train_price = (
        price_model.loc[
            price_model[ID_COL].isin(train_ids)
        ]
        .copy()
        .sort_values([ID_COL, "price_date"])
        .reset_index(drop=True)
    ...
    test_price = (
        price_model.loc[
            price_model[ID_COL].isin(test_ids)
        ]
        .copy()
        .sort_values([ID_COL, "price_date"])
        .reset_index(drop=True)
...
def prepare_raw_splits(
    ...
    structural = validate_keys_and_population(
        client,
        price,
    )
    split = split_customer_data(
        client,
        structural["price_model"],
        test_size=test_size,
        random_state=random_state,
    )
...
def validate_consumption_for_log(
...
def create_client_features(
    ...
    # 문자열 MISSING은 정보 부재를 나타내는 범주로 유지한다.
...
def aggregate_price_features(
    ...
    work = (
        work.loc[
            work["price_date"] < reference_date
        ]
        .sort_values([ID_COL, "price_date"])
        .copy()
    )
    ...
    recent_start_date = (
        reference_date
        - pd.DateOffset(months=recent_months)
    )
    recent_mask = (
        work["price_date"] >= recent_start_date
    )
    previous_mask = (
        work["price_date"] < recent_start_date
    )
...
def find_exact_duplicate_columns(
    ...
    for current_index, current_col in enumerate(
        columns
    ):
        for previous_col in columns[:current_index]:
            if df[current_col].equals(
                df[previous_col]
            ):
                duplicate_map[
                    current_col
...
def validate_result_frames(
    ...
    numeric_train = train_result.select_dtypes(
        include=np.number
    )
    numeric_test = test_result.select_dtypes(
        include=np.number
    )
    ...
    validation_summary = pd.DataFrame({
        ...
        "값": [
            ...
            len(
                set(train_result[ID_COL])
                & set(test_result[ID_COL])
            ),
            int(
                np.isinf(
    ...
    assert train_result.columns.tolist() == (
        test_result.columns.tolist()
    )
...
def build_model_datasets_from_splits(
    ...
    if export:
        if interim_dir is None or processed_dir is None:
            raise ValueError(
                "export=True이면 interim_dir와 processed_dir가 모두 필요합니다."
            )
    ...
    consumption_check = (
        validate_consumption_for_log(
            train_client
        )
    )

    train_client_fe = create_client_features(
        train_client
    )
    test_client_fe = create_client_features(
        test_client
    )

    assert len(train_client_fe) == len(
        train_client
    )
    assert len(test_client_fe) == len(
        test_client
    )
    assert (
        train_client_fe.columns.tolist()
        == test_client_fe.columns.tolist()
    )

    train_price_agg = aggregate_price_features(
        train_price
    )
    test_price_agg = aggregate_price_features(
        test_price
    )
    ...
    assert set(
        train_price_agg[ID_COL]
    ) == set(train_client[ID_COL])
    assert set(
        test_price_agg[ID_COL]
    ) == set(test_client[ID_COL])
    ...
    train_featured = create_cross_source_features(
        train_merged
    )
    test_featured = create_cross_source_features(
        test_merged
    )

    assert (
        train_featured.columns.tolist()
        == test_featured.columns.tolist()
    )
    assert len(train_featured) == len(
        train_client
    )
    assert len(test_featured) == len(
        test_client
    )
    ...
    # 중간 병합·파생 데이터는 data/interim에 저장한다.
    # data/processed는 이 interim CSV를 다시 읽은 복사본에서
    # 중복 제거, 날짜 제외, 무한대 정리 등 최종 정제를 수행해 만든다.
    if export:
        train_interim_path = (
            interim_output_dir / "02_train_merged.csv"
        )
        test_interim_path = (
            interim_output_dir / "02_test_merged.csv"
        )

        train_featured.to_csv(
            train_interim_path,
            index=False,
            encoding="utf-8-sig",
        )
        test_featured.to_csv(
            test_interim_path,
            index=False,
            encoding="utf-8-sig",
        )
        ...
        # processed 데이터가 실제로 interim 파일에서 출발하도록 재로드한다.
        train_featured = pd.read_csv(train_interim_path)
        test_featured = pd.read_csv(test_interim_path)

        for col in CLIENT_DATE_COLS:
            train_featured[col] = pd.to_datetime(
                train_featured[col],
                errors="coerce",
            )
            test_featured[col] = pd.to_datetime(
                test_featured[col],
                errors="coerce",
            )
    ...
    margin_equal_rate = float(
        margin_equal_mask.mean()
    )

    known_redundant_cols: list[str] = []
    if margin_equal_rate >= 0.999:
        known_redundant_cols.append(
            "margin_gross_pow_ele"
        )
    ...
    X_train_final = train_featured[
        base_feature_columns
    ].copy()
    X_test_final = test_featured[
        base_feature_columns
    ].copy()
    ...
    if export:
        train_result_path = (
            processed_output_dir / "train.csv"
        )
        test_result_path = (
            processed_output_dir / "test.csv"
        )
    ...
    interim_export_summary = pd.DataFrame({
        ...
        "저장 경로": [
            str(path.resolve())
            for path in interim_paths.values()
        ],
    })
...
def _print_run_summary(
    ...
    print("[분할 구조]")
    print(
        artifacts["split_structure"]
        .to_string(index=False)
    )
    print("\n[정제 요약]")
    print(
        artifacts["cleaning_summary"]
        .to_string(index=False)
    )
    print("\n[interim 저장 요약]")
    print(
        artifacts["interim_export_summary"]
        .to_string(index=False)
    )
    print("\n[processed 저장 요약]")
    print(
        artifacts["export_summary"]
        .to_string(index=False)
    )
    print("\n원-핫 인코딩: 미적용")
